      ...
      <section v-else class="json-data-view">
        ...
        <div v-else-if="jsonViewMode === 'detail'" class="tripadvisor-detail">
         ...
         <div class="info-box">
            ...
            <div class="ai-briefing-box" v-if="selectedJsonItem">
              <div class="ai-header">
                <span class="ai-icon">✨</span> AI 장소 브리핑
              </div>
              <div v-if="isAiLoading" class="ai-content loading-text">
                AI가 해당 장소의 매력 포인트를 요약하고 있습니다... 🤖
              </div>
...
// 💡 OpenAI API 관련 변수들을 최상단으로 옮겨 모든 컴포넌트와 함수에서 안전하게 접근하도록 합니다.
const OPENAI_API_KEY = import.meta.env.VITE_OPENAI_API_KEY;
const AI_MODEL = "gpt-5-mini"; // 공공기관 실습 API 서버 전용 고정 모델
...
const globalSearchResults = computed(() => {
  ...
  const boardRes = freeBoardPosts.value.filter(post => 
    ...
    addr1: post.content.length > 20 ? post.content.substring(0, 20) + '...' : post.content, 
    originalPost: post
  }));
  ...
  const scheduleRes = schedules.value.filter(s => 
    ...
    _category: '일정', contentid: s.id, title: s.title, addr1: `일정: ${s.startDate} ~ ${s.endDate}`, 
    originalPost: s
  }));
...
const loadAllJsonForSearch = async () => {
  ...
  for (const cat of targetCategories) {
    try {
      ...
      console.warn(`[통합검색] ${cat} 파일 로드 실패.`);
    }
  }
...
const aiBriefingText = ref('');
const isAiLoading = ref(false);

const openJsonDetail = (item) => { 
  selectedJsonItem.value = item;
  jsonViewMode.value = 'detail'; 
  fetchAiBriefing(item.title); // ✨ 상세창 열릴 때 AI 요약 실행!
};

const fetchAiBriefing = async (placeName) => {
  aiBriefingText.value = '';
  isAiLoading.value = true;
  
  // 환경변수에서 키값을 안전하게 가져옵니다.
  const apiKey = import.meta.env.VITE_OPENAI_API_KEY; 
  
  if (!apiKey) {
    aiBriefingText.value = 'API 키가 없어 요약을 불러올 수 없습니다.';
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-5-mini",
        // 🚨 OpenAI 서버가 정확하게 요구한 최신 파라미터명으로 변경!
        max_completion_tokens: 2000, 
        messages: [
          { 
            role: "system", 
            content: "너는 경북 구미 지역의 전문 여행 가이드야. 사용자가 주는 장소 이름에 대해 특징, 역사, 즐길 거리 등을 2~3문장(약 100자 내외)으로 아주 매력적이고 자연스럽게 요약해줘. 해요체를 사용해." 
          },
    ...
    if (!response.ok) {
      ...
      throw new Error(`API 에러: ${errorData.error?.message || response.status}`);
    }
    ...
    if (reply) {
      aiBriefingText.value = reply;
    } else {
      aiBriefingText.value = "AI가 해당 장소에 대한 상세한 매력 포인트를 찾지 못했습니다. 🥲";
    }

  } catch (error) {
    console.error("🚨 AI 브리핑 에러 발생:", error);
    aiBriefingText.value = "AI 요약을 불러오는 중 일시적인 네트워크 오류가 발생했습니다.";
  } finally {
    isAiLoading.value = false;
  }
};
...
const getChipClass = (item, date) => { 
  const isStart = item.startDate === date;
  const isEnd = item.endDate === date; 
  if (isStart && isEnd) return 'chip-single'; 
  if (isStart) return 'chip-start'; 
  if (isEnd) return 'chip-end';
  return 'chip-middle'; 
};
...
const saveSchedule = () => {
  ...
  if (selectedSchedule.value) { 
    ...
    if (idx !== -1) schedules.value[idx] = { ...scheduleForm.value }; 
  } else { 
    schedules.value.push({ ...scheduleForm.value, id: Date.now(), isLiked: false, likeCount: 0, category: '일정' });
  }
  closeModals();
};
...
// 💡 새로운 기능이 완벽하게 이식된 댓글 컴포넌트 선언
const CommentSection = defineComponent({
  props: ['targetId'],
  setup(props) {
    const newComment = ref({ text: '', image: '', password: '' });
    const targetComments = computed(() => { 
      return globalComments.value.filter(c => c.targetId === props.targetId).sort((a, b) => b.createdAt - a.createdAt); 
    });
    const handleImg = (e) => handleImageUpload(e, newComment.value);

    // AI 리뷰 분석 관련 변수
    const aiReviewBriefingText = ref('');
    const isReviewLoading = ref(false);

    // AI 리뷰 요약 실행 함수
    const generateReviewBriefing = async () => {
      const reviewsList = targetComments.value.map(c => c.text);
      
      if (reviewsList.length === 0) {
        alert("분석할 댓글이 없습니다. 먼저 댓글을 등록해 주세요!");
        return;
      }

      aiReviewBriefingText.value = '';
      isReviewLoading.value = true;

      const reviewsText = reviewsList.map(review => `- ${review}`).join('\n');
      ...
      try {
        const response = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: "너는 데이터 기반의 리뷰 분석 전문가야. 제공된 고객 리뷰들을 분석하여 1) 소비자들이 가장 만족하는 점(장점), 2) 개선이 필요한 아쉬운 점(단점), 3) 한 줄 총평을 명확하게 요약해줘. 친절한 해요체 형태의 가독성 좋은 리포트 형식으로 작성해라."
              },
              {
                role: "user",
                content: `다음은 우리 매장(또는 상품)의 실제 고객 리뷰 목록이다:\n\n${reviewsText}`
              }
            ],
            verbosity: "medium"
          })
        ...
        const data = await response.json();
        aiReviewBriefingText.value = data.choices[0].message.content;
      } catch (error) {
        console.error("🚨 리뷰 분석 에러:", error);
        aiReviewBriefingText.value = "리뷰를 분석하는 중 일시적인 오류가 발생했습니다.";
      } finally {
    ...
    const submit = () => {
      ...
      globalComments.value.push({ 
        id: Date.now(), 
        targetId: props.targetId, 
        text: newComment.value.text, 
        image: newComment.value.image, 
    ...
    const deleteComm = (comment) => {
      ...
      if (pwd === comment.password) { 
        globalComments.value = globalComments.value.filter(c => c.id !== comment.id);
      } else if (pwd !== null) { 
    ...
    return () => h('div', { class: 'comment-section' }, [
      h('div', { class: 'comment-header-row' }, [
        h('h3', `댓글 (${targetComments.value.length})`),
        targetComments.value.length > 0 
          ? h('button', { class: 'btn-ai-analyze', onClick: generateReviewBriefing }, '✨ AI 리뷰 트렌드 분석') 
          : null
      ]),

      isReviewLoading.value 
        ? h('div', { class: 'ai-review-report-box loading-state' }, '🤖 AI 리뷰 분석가가 등록된 댓글들을 종합 분석하고 있습니다...') 
        : null,
      aiReviewBriefingText.value && !isReviewLoading.value
        ? h('div', { class: 'ai-review-report-box' }, [
            h('div', { class: 'ai-report-title' }, '📊 AI 고객 의견 트렌드 리포트'),
            h('p', { class: 'ai-report-content' }, aiReviewBriefingText.value)
          ]) 
        : null,
...
/* ✨ AI 브리핑 박스 디자인 추가 ✨ */
.ai-briefing-box { margin-top: 20px; background: linear-gradient(145deg, #f0f7ff 0%, #ffffff 100%); border: 1px solid #cce5ff; border-radius: 12px; padding: 16px; box-shadow: 0 4px 15px rgba(0, 123, 255, 0.05); }
.ai-header { font-size: 1rem; font-weight: 800; color: #0056b3; margin-bottom: 10px; display: flex; align-items: center; gap: 6px; }
.ai-icon { font-size: 1.2rem; }
.ai-content { font-size: 0.95rem; line-height: 1.6; color: #444; word-break: keep-all; }
.loading-text { color: #888; font-style: italic; animation: pulse 1.5s infinite; }
...
.btn-ai-analyze { background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); color: white; border: none; padding: 6px 14px; border-radius: 20px; font-size: 0.85rem; font-weight: bold; cursor: pointer; transition: 0.3s; box-shadow: 0 2px 8px rgba(0,123,255,0.25); }
.btn-ai-analyze:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(0,123,255,0.4); }

/* ✨ AI 리뷰 보고서 카드 디자인 ✨ */
.ai-review-report-box { margin-bottom: 25px; padding: 18px; border-radius: 12px; background: #fafaff; border: 1.5px dashed #007bff; box-shadow: inset 0 2px 6px rgba(0, 123, 255, 0.03); }
.ai-review-report-box.loading-state { color: #666; font-style: italic; font-weight: bold; text-align: center; background: #fcfcff; border-color: #adb5bd; animation: aiPulse 1.5s infinite; }
.ai-report-title { font-size: 1.05rem; font-weight: 800; color: #0056b3; margin-bottom: 12px; display: flex; align-items: center; gap: 6px; }
.ai-report-content { font-size: 0.95rem; line-height: 1.7; color: #333; white-space: pre-wrap; word-break: keep-all; margin: 0; }
